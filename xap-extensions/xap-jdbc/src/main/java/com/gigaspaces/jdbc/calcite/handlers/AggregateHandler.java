    ...
    public void apply(GSAggregate gsAggregate, QueryExecutor queryExecutor){
        ...
        List<String> fields = child.getRowType().getFieldNames();
            for (ImmutableBitSet groupSet : gsAggregate.groupSets) {
                ...
                groupSet.forEach(bit -> {
                    ...
                    TableContainer table = queryExecutor.isJoinQuery() ? queryExecutor.getTableByColumnIndex(bit) : queryExecutor.getTableByColumnName(columnName);
        ...
        for (AggregateCall aggregateCall : gsAggregate.getAggCallList()) {
            ...
            if(allColumns){
                ...
                final TableContainer table = queryExecutor.isJoinQuery() ? queryExecutor.getTableByColumnIndex(aggregateCall.getArgList().get(0)) : queryExecutor.getTableByColumnName(column);
                final IQueryColumn queryColumn = queryExecutor.isJoinQuery() ? queryExecutor.getColumnByColumnIndex(aggregateCall.getArgList().get(0)) : table.addQueryColumnWithoutOrdinal(column, null, false);
                queryExecutor.addColumn(queryColumn, false);
                aggregationColumn = new AggregationColumn(aggregationFunctionType, String.format("%s(%s)", aggregateCall.getAggregation().getName().toLowerCase(Locale.ROOT), column), queryColumn, true, false, columnCounter.getAndIncrement());
